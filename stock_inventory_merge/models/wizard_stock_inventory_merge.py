...
class WizardStockInventoryMerge(models.TransientModel):
    ...
    def action_merge(self):
        ...
        if len(context.get('active_ids', [])) < 2 or\
                context.get('active_model', False) != 'stock.inventory':
            raise UserError(_(
                "Please select at least two or more inventories to merge"))
        inventories = inventory_obj.browse(context.get('active_ids', []))

        # Check valid state
        if inventories.filtered(lambda x: x.state != 'confirm'):
            raise UserError(_(
                "Merging is only allowed on 'In Progress' inventories."))
